...
public class ProductSearchDOMMFTest extends ParallelizedTestBase {
	public ProductSearchDOMMFTest(String browserName, String version,
		...
		driver = DriverScriptRegistry.getDriverScript(MessageFormat.format(
				"{0}:{1}:{2}:{3}:{4}", browserName, version, platform, os, this
						.getClass().getName()), "ProductSearch_DOM_MF");
	}
	...
	public void testAddToCartWEXPickupDeliveryAvailableShippingUnavailable() {
		...
		driver.setCurrentTestCaseDescription("PS_AZ_Add to Cart_WEX_Pickup, Delivery Available_Shipping Unavailable_Change Store_Pickup");
		driver.driveTestExecution();
	}
