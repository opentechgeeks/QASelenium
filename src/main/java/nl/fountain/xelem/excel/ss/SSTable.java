...
public class SSTable extends AbstractXLElement implements Table {
	...
	public Row removeRowAt(int rowIndex) {
		return rows.remove(new Integer(rowIndex));
	}
	...
	public Element assemble(Element parent, GIO gio) {
		...
		if (getStyleID() != null) {
			te.setAttributeNodeNS(createAttributeNS(doc, "StyleID",
					getStyleID()));
			gio.addStyleID(getStyleID());
		}
		if (rowheight > 0.0)
			te.setAttributeNodeNS(createAttributeNS(doc, "DefaultRowHeight", ""
					+ rowheight));
		if (columnwidth > 0.0)
			te.setAttributeNodeNS(createAttributeNS(doc, "DefaultColumnWidth",
					"" + columnwidth));
	...
	void setExpandedColumnCount(String s) {
	...
	// method called by ExcelReader
	void setExpandedRowCount(String s) {
