...
public class SSRow extends AbstractXLElement implements Row {
	...
	public Element assemble(Element parent, GIO gio) {
		...
		if (idx != 0)
			re.setAttributeNodeNS(createAttributeNS(doc, "Index", idx));
		if (getStyleID() != null) {
			re.setAttributeNodeNS(createAttributeNS(doc, "StyleID",
					getStyleID()));
			gio.addStyleID(getStyleID());
		}
		if (span > 0)
			re.setAttributeNodeNS(createAttributeNS(doc, "Span", span));
	...
	private void setAdditionalAttributes(Document doc, Element rowElement) {
		if (height > 0.0)
			rowElement.setAttributeNodeNS(createAttributeNS(doc, "Height", ""
					+ height));
		if (hidden)
			rowElement
					.setAttributeNodeNS(createAttributeNS(doc, "Hidden", "1"));
	}
	...
	public Element createElement(Document doc) {
		...
		if (getStyleID() != null) {
			rowElement.setAttributeNodeNS(createAttributeNS(doc, "StyleID",
					getStyleID()));
		}
	...
	/**
	 * Sets the value of the ss:Index-attribute of this Row-element. This method
	 * is called by {@link nl.fountain.xelem.excel.Table#rowIterator()} to set
	 * the index of this row during assembly.
	 */
