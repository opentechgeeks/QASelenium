 ...
 * <P>
 * This class can be used to add rows, columns and cells to the worksheet. The
 * addCell-methods add a cell at the current position of the worksheet's
 * cellPointer (see {@link #getCellPointer()}) and move the cellPointer. If you
 * whish to obtain the address of the newly added cell, it should be obtained
 * from the cellPointer prior to the add:
 * 
 * <PRE>
 ...
 * </PRE>
 * 
 * The addCellAt-methods, which take an address, a string or a row/column pair
 * of ints as parameter, move the cellPointer to the specified position, add a
 * cell and leave the cellPointer pointing to the next position.
 */
public class SSWorksheet extends AbstractXLElement implements Worksheet {
	...
	/**
	 * Constructs a new SSWorksheet with the given name. The worksheets
	 * cellpointer will be at position row 1, column 1.
	 * 
	...
	public Cell addCellAt(String a1_ref) {
		return addCellAt(Address.calculateRow(a1_ref),
				Address.calculateColumn(a1_ref));
	}
	...
	public Cell addCellAt(String a1_ref, Cell cell) {
		return addCellAt(Address.calculateRow(a1_ref),
				Address.calculateColumn(a1_ref), cell);
	}
	...
	public Cell getCellAt(String a1_ref) {
		return getCellAt(Address.calculateRow(a1_ref),
				Address.calculateColumn(a1_ref));
	}
	...
	public Cell removeCellAt(String a1_ref) {
		return removeCellAt(Address.calculateRow(a1_ref),
				Address.calculateColumn(a1_ref));
	}
	...
	public boolean hasCellAt(String a1_ref) {
		return hasCellAt(Address.calculateRow(a1_ref),
				Address.calculateColumn(a1_ref));
	}
	...
	public Element assemble(Element parent, GIO gio) {
		...
		if (protect)
			wse.setAttributeNodeNS(createAttributeNS(doc, "Protected", "1"));
		if (righttoleft)
			wse.setAttributeNodeNS(createAttributeNS(doc, "RightToLeft", "1"));
