 ...
 * <P>
 * The {@link #setData(Object)}-method reflects upon the class of the given
 * object and will delegate to a corresponding setData-method if such a method
 * is available. If no corresponding method is found this method sets the data
 * of this cell to the <code>toString</code>-value of the given object and the
 * datatype to "String". This class may be extended to accommodate
 * setData-methods for java-objects that might otherwise be displayed as the
 * <code>toString</code>-value of that object.
 * 
 * <P id="nullvalues">
 * <b>Null values</b><br>
 ...
 * the resulting xml will have a datatype set to "String" and the cell will
 * display "Infinite" when the spreadsheet is opened.
 * 
 * <P id="nanvalues">
 * <b>NaN values</b><br>
 * If the passed parameter is of type Double, Float or the primitive
...
public class SSCell extends AbstractXLElement implements Cell {
	...
	/**
	 * Creates a new SSCell with an initial datatype of "String" and an empty
	 * ("") value.
	 * 
	 * @see nl.fountain.xelem.excel.Worksheet#addCell()
	 */
	 ...
	 * ss:StyleID-attribute is not deployed in the resulting xml and the
	 * Default-style will be employed on the cell.
	 * <P>
	 * The refered style-definition must be available from the
	 * {@link nl.fountain.xelem.XFactory}. If the definition was not found, the
	 * {@link XLWorkbook}-implementation will create an empty
	 * ss:Style-definition and adds a UnsupportedStyleException-warning.
	 * 
	 * @param id
	...
	public Element assemble(Element parent, GIO gio) {
		...
		if (formula != null)
			ce.setAttributeNodeNS(createAttributeNS(doc, "Formula", formula));
		if (href != null)
			ce.setAttributeNodeNS(createAttributeNS(doc, "HRef", href));
		if (mergeacross > 0)
			ce.setAttributeNodeNS(createAttributeNS(doc, "MergeAcross",
					mergeacross));
		if (mergedown > 0)
			ce.setAttributeNodeNS(createAttributeNS(doc, "MergeDown", mergedown));
	 ...
	 * 
	 * @param type
	 *            Must be one of Cell's DATATYPE_XXX values.
	 */
	protected void setType(String type) {
		if (DATATYPE_BOOLEAN.equals(type) || DATATYPE_DATE_TIME.equals(type)
				|| DATATYPE_ERROR.equals(type) || DATATYPE_NUMBER.equals(type)
