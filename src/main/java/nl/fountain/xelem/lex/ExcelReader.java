 ...
 * <P>
 * An ExcelReader can deliver the contents of an xml-file or an xml-InputSource
 * as a fully populated {@link nl.fountain.xelem.excel.Workbook}.
 * <P>
 * Furthermore, it can dispatch events, values and instances of
 * {@link nl.fountain.xelem.excel.XLElement XLElements} to listeners registered
 * at this ExcelReader while parsing an xml-file or xml-InputSource.
 * 
...
public class ExcelReader {
	...
	/**
	 * Constructs a new ExcelReader. Obtains a
	 * {@link javax.xml.parsers.SAXParser} from an instance of
	 * {@link javax.xml.parsers.SAXParserFactory} to do the parsing. There is no
	 * need to configure factory parameters.
	 * 
	 * @throws ParserConfigurationException
	 *             if a parser cannot be created which satisfies the current
	 *             configuration
	 * @throws SAXException
	 *             for SAX errors
	 */
	...
	/**
	 * Constructs a new ExcelReader. Uses the given parser. The parser should be
	 * namespace aware.
	 * 
	 * @param parser
	...
	/**
	 * Returns the SAXParser that is used by this ExcelReader. (Just to see what
	 * we've got under the hood.)
	 * <P>
	 * Under Java 1.4 we might see a org.apache.crimson.jaxp.SAXParserImpl, it
	 * could be a com.sun.org.apache.xerces.internal.jaxp.SAXParserImpl under
	 * Java 1.5.
	 * 
	...
	/**
	 * Sets the area that is to be read. Reading will be restricted to the
	 * specified area until another area is set or until the read area has been
	 * cleared.
	 * 
	 * @param area
	 *            the area to read
	...
	/**
	 * Clears the read area. Reading will not be rerstricted after a call to
	 * this method.
	 * 
	 */
	 ...
	 * 
	 * @param listener
	 *            the ExcelReaderListener to be registered
	...
	/**
	 * Delivers the contents of the specified file as a fully populated
	 * Workbook. If a read area was set on this ExcelReader its worksheets are
	 * only populated in the specified area. If listeners are registered, this
	 * ExcelReader dispatches events to these listeners during the read.
	 * Performs a read.
	 * 
	 * @param fileName
	 ...
	 * @throws SAXException
	 *             signals a general SAX error or warning
	 * 
	 * @see #read(String)
	 */
	...
	/**
	 * Delivers the contents of the given InputSource as a fully populated
	 * Workbook. If a read area was set on this ExcelReader its worksheets are
	 * only populated in the specified area. If listeners are registered, this
	 * ExcelReader dispatches events to these listeners during the read.
	 * Performs a read.
	 ...
	 */
	public Workbook getWorkbook(InputSource source) throws IOException,
			SAXException {
	 ...
	 * on this ExcelReader, only events occuring within the limits of the area
	 * are dispatched.
	 * <P>
	 ...
	 * under java 1.4 and previous releases. Releases prior to java 1.5 will
	 * throw a <code>org.xml.sax.SAXParseException</code> under these adverse
	 * conditions.
	 * 
	 * @param fileName
	 *            the name of the file to be read
	...
	/**
	 * Reads the stream of the given InputSource and dispatches events to
	 * registered {@link ExcelReaderListener ExcelReaderListeners}. If a read
	 * area was set on this ExcelReader, only events occuring within the limits
	 * of the area are dispatched.
	 * <P>
	...
	/**
	 * Gets a map of prefixes (keys) and uri's recieved while reading. May be
	 * obtained after a read. Performing a new read will clear any previously
	 * recorded entries from the map.
	 * 
